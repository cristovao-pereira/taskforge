            ...
            {decisions.length === 0 ? (
              ...
              decisions.map((decision) => (
                <div key={decision.id} className="relative pl-10 py-4 group">
                  {/* Timeline Dot */}
                  <div className={`absolute left-[16px] top-8 w-1.5 h-1.5 rounded-full ring-4 ring-zinc-950 ${
                    decision.riskLevel === 'high' ? 'bg-orange-500' : 
                  ...
                  <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-4 hover:bg-zinc-900 hover:border-zinc-700 transition-all">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider border ${
                         decision.riskLevel === 'high' ? 'bg-orange-500/5 text-orange-500 border-orange-500/10' : 
                    ...
                    <div className="mt-3 flex items-center gap-2">
                        {!decision.outcome || decision.outcome === 'Pending' ? (
                          ...
                          <div className={`flex items-center gap-1.5 text-xs font-medium ${
                            decision.outcome === 'Positive' ? 'text-emerald-500' :
             ...
             <div className="space-y-4">
                <div className="space-y-1">
                   <div className="flex justify-between text-xs text-zinc-500">
                      ...
                      <span className="text-emerald-500">+{metrics.successRate > 50 ? '12%' : '0%'}</span>
                   </div>
                   <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${metrics.successRate}%` }}></div>
                   </div>
                </div>
                ...
                <div className="space-y-1">
                   <div className="flex justify-between text-xs text-zinc-500">
                      ...
                      <span className="text-orange-500">-{metrics.highRisk < 40 ? '5%' : '0%'}</span>
                   </div>
                   <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full bg-orange-500 rounded-full" style={{ width: `${metrics.highRisk}%` }}></div>
                   </div>
                </div>
             </div>
          </section>
