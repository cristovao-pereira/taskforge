...
import { loginWithGoogle, loginWithGithub, loginWithEmail, syncUserWithBackend } from '../lib/firebase';
    ...
    <div className="min-h-screen flex flex-col font-sans relative overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
      ...
      <nav className="w-full py-6 px-6 md:px-12 flex justify-between items-center absolute top-0 left-0 z-50">
        ...
        <div className="hidden md:block">
          <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>Não tem uma conta? <Link to="/signup" className="hover:opacity-80 ml-1 font-medium" style={{ color: 'var(--accent-color)' }}>Criar conta</Link></span>
        </div>
      </nav>
      ...
      <main className="flex-grow flex items-center justify-center relative p-4 z-10">
        <div className="w-full max-w-md shadow-2xl rounded-2xl p-8 md:p-10 backdrop-blur-sm" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
          <div className="text-center mb-8">
            <h1 className="text-2xl md:text-3xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Bem-vindo de volta</h1>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Entre para gerenciar seus Super Agentes de IA</p>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleEmailLogin}>
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-xs uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Ou continue com</span>
              <div className="flex-grow" style={{ borderTop: '1px solid var(--border-color)' }}></div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="block text-sm font-medium" htmlFor="password" style={{ color: 'var(--text-secondary)' }}>Senha</label>
                <button type="button" onClick={() => toast.info('Recuperação de senha disponível em breve.')} className="text-xs font-medium hover:opacity-80" style={{ color: 'var(--accent-color)' }}>Esqueceu sua senha?</button>
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
      ...
      <footer className="w-full py-6 text-center z-10">
        ...
        <div className="mt-2 flex justify-center gap-4 text-xs">
          <Link to="/como-funciona" className="hover:opacity-70 transition-colors" style={{ color: 'var(--text-secondary)' }}>Termos</Link>
          <Link to="/precos" className="hover:opacity-70 transition-colors" style={{ color: 'var(--text-secondary)' }}>Privacidade</Link>
          <Link to="/como-funciona" className="hover:opacity-70 transition-colors" style={{ color: 'var(--text-secondary)' }}>Ajuda</Link>
        </div>
      </footer>
    </div>
