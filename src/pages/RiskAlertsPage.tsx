      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="space-y-8">
          ...
          <section className="card-standard">
            ...
            <div className="space-y-4">
              {insights.length === 0 ? (
                <div className="p-4 text-center border border-zinc-800/50 bg-zinc-900/30 rounded-lg">
                  <p className="text-zinc-500 text-xs">As sugestões aparecerão aqui conforme você interagir com os módulos forge e sua estratégia evoluir.</p>
                </div>
              ) : (
            ...
            <div className="card-standard divide-y divide-zinc-800/50">
              {history.length === 0 ? (
                <div className="p-4 text-center">
                  <p className="text-zinc-500 text-xs mt-2 mb-2">Nenhum alerta resolvido ainda.</p>
                </div>
              ) : (
                history.map((item) => (
...
const RiskCard: React.FC<{ risk: any, onResolve: () => void, onViewPlan: () => void, onAdjust: () => void }> = ({ risk, onResolve, onViewPlan, onAdjust }) => {
  ...
  return (
    <div className={`card-standard group ${isCritical ? 'border-orange-500/20 shadow-lg shadow-orange-500/5' : ''
      }`}>
